...
alter table public.customer_orders
  add constraint customer_orders_nearest_landmark_check check (
    nearest_landmark is null or nearest_landmark in (
...
revoke all on function public.create_customer_order(jsonb,uuid,text,text,text,text,text,uuid,text) from public, anon, authenticated;
drop function public.create_customer_order(jsonb,uuid,text,text,text,text,text,uuid,text);
...
create or replace function public.create_customer_order(
  ...
  p_delivery_zone_id uuid default null, p_nearest_landmark text default null,
...
begin
  ...
  if p_fulfillment_type = 'DELIVERY' then
    if p_nearest_landmark is null or p_nearest_landmark not in ('Acada', 'Park', 'Male Hostels', 'Female Hostels', 'Joints', 'Complex Area', 'Remedial Studies', 'Faculties', 'Commercial Area', 'Postgraduate School', 'UNIMAID Garden', 'El-Kanemi Hall', 'Student Centre', 'NUGA', 'Senate Building', 'Other') then
      return jsonb_build_object('success', false, 'code', 'DELIVERY_LANDMARK_REQUIRED', 'message', 'Select a valid nearest landmark.'); end if;
  ...
  for v_item in select value from jsonb_array_elements(p_items) loop
    ...
    if not coalesce(v_product.is_active, false) or not v_product.is_in_stock or (v_product.stock_quantity is not null and v_product.stock_quantity < v_quantity) then
      return jsonb_build_object('success', false, 'code', 'PRODUCT_UNAVAILABLE', 'message', v_product.name || ' is currently unavailable. Please remove it from your cart and try again.', 'product_id', v_product_id); end if;
  ...
  insert into public.customer_orders (order_number,customer_user_id,customer_name,customer_phone,campus_id,fulfillment_type,delivery_zone_id,nearest_landmark,delivery_location,notes,subtotal_kobo,delivery_fee_kobo,service_fee_kobo,total_kobo,tracking_token_hash) values (v_order_number,v_customer_user_id,trim(p_customer_name),trim(p_customer_phone),p_campus_id,p_fulfillment_type,case when p_fulfillment_type='DELIVERY' then p_delivery_zone_id end,case when p_fulfillment_type='DELIVERY' then p_nearest_landmark end,case when p_fulfillment_type='DELIVERY' then trim(p_delivery_location) end,nullif(trim(coalesce(p_notes,'')),''),v_subtotal_kobo,v_delivery_fee_kobo,v_service_fee_kobo,v_total_kobo,crypt(v_tracking_token,gen_salt('bf',12))) returning id into v_customer_order_id;
