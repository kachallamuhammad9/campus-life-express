-- Enforce the product lifecycle and temporary availability state at the
-- authoritative checkout boundary.  All validation precedes any order-number
-- allocation or inserts, so rejected carts leave no partial order/payment data.
