...
export function buildWhatsAppOrderMessage(response, customer, campusName) {
    ...
    return [
        ...
        response.fulfillment_type === 'DELIVERY' && response.nearest_landmark ? `Landmark: ${response.nearest_landmark}` : '',
        response.fulfillment_type === 'DELIVERY' && response.delivery_location ? `Exact Location: ${response.delivery_location}` : '', '',
